...
public struct SettingsFeature {
  /// Lifecycle of the bundled `tty7` CLI install. Lives on the
  ...
  public struct State: Equatable {
    ...
    public init(settings: GlobalSettings = .default) {
      ...
      defaultWorktreeBaseDirectoryPath =
        Tty7Paths.normalizedWorktreeBaseDirectoryPath(settings.defaultWorktreeBaseDirectoryPath) ?? ""
    }
    ...
    var globalSettings: GlobalSettings {
      GlobalSettings(
        ...
        defaultWorktreeBaseDirectoryPath: Tty7Paths.normalizedWorktreeBaseDirectoryPath(
          defaultWorktreeBaseDirectoryPath
        ),
    ...
    Reduce { state, action in
      ...
      case .showNotificationPermissionAlert(let errorMessage):
        ...
        if let errorMessage, !errorMessage.isEmpty {
          message =
            "tty7 cannot send system notifications.\n\n"
            + "Error: \(errorMessage)"
        } else {
          message = "tty7 cannot send system notifications while permission is denied."
        }
      ...
      case .requestAutoDeleteDaysChange(let newPeriod):
        ...
        // override wired in `tty7App`, which bridges the
