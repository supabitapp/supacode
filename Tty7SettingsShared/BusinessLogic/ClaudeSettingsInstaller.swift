...
nonisolated struct ClaudeSettingsInstaller {
  ...
  /// duplicates left by older tty7 versions.
...
nonisolated enum ClaudeSettingsInstallerError: Error, Equatable, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .invalidJSON(let detail):
      "Claude settings must be valid JSON before tty7 can install hooks (\(detail))."
    case .invalidRootObject:
      "Claude settings must be a JSON object before tty7 can install hooks."
    }
  }
}
