...
///   same host (gated on `TTY7_SOCKET_PATH`); it feeds the app's liveness
...
/// `TTY7_SURFACE_ID` so it no-ops outside a tty7 surface.
///
...
public nonisolated enum AgentPresenceOSC {
  /// Env var present only on tty7 surfaces, so its presence is the
  /// no-op-outside-tty7 emit gate.
  public static let surfaceEnvVar = "TTY7_SURFACE_ID"
  ...
  public struct Signal: Equatable, Sendable {
    ...
    /// The agent's LOCAL process id. The emit gates it on `TTY7_SOCKET_PATH`
  ...
  /// stdout. The caller guards emission on `TTY7_SURFACE_ID` and runs
  /// `ttyResolveSnippet` first.
  ///
  /// The pid suffix is gated on `TTY7_SOCKET_PATH` (set only on the local
  ...
  static func emitShell(event: HookEvent, agent: SkillAgent) -> String {
    ...
    return #"__sp=""; [ -n "${TTY7_SOCKET_PATH:-}" ] && __sp=";\#(pidField)=$PPID"; "#
