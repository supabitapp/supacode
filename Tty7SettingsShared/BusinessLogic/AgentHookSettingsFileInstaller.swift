...
nonisolated struct AgentHookSettingsFileInstaller {
  ...
  /// Compare the set of tty7-managed commands present in the settings
  /// file against the expected (canonical) set:
  /// - `.installed`     — actual tty7 commands == expected, no extras
  /// - `.notInstalled`  — no tty7-managed commands at all
  /// - `.outdated`      — some present, but the set differs (extras, missing,
  ///                      or stale variants from older tty7 versions)
  func installState(
    settingsURL: URL,
    ...
    do {
      ...
      let actual = Self.installedTty7Commands(in: settingsObject)
  ...
  /// All tty7-marked `command` strings under the `hooks` map. Filters
  /// via `AgentHookCommandOwnership` so user-authored hooks are never
  /// treated as "ours."
  private static func installedTty7Commands(
      ...
      for group in groups {
        ...
        for hook in hooks {
          guard let hookObject = hook.objectValue,
            ...
            AgentHookCommandOwnership.isTty7ManagedCommand(command)
          else { continue }
  ...
  /// Removes every tty7-managed command (current and legacy) from the
  /// settings file. User-authored hooks are preserved — the trailing
  /// `# tty7-managed-hook` sentinel is the source of truth for
  ...
  func uninstall(
    ...
    let pruned = try pruneAllTty7Commands(from: hooksObject)
    settingsObject["hooks"] = .object(pruned)
    try writeSettings(settingsObject, to: settingsURL)
  }

  /// `install = uninstall + append`: strip every tty7-managed entry from
  ...
  func install(
    ...
    var pruned = try pruneAllTty7Commands(from: existing)
  ...
  private func pruneAllTty7Commands(
    ...
    for (event, value) in hooksObject {
      ...
      let filtered = groups.compactMap { stripAllTty7Commands(from: $0) }
      if !filtered.isEmpty {
  ...
  /// Strip every tty7-managed command from the group. User-authored
  /// hooks (no `# tty7-managed-hook` sentinel) survive untouched.
  private func stripAllTty7Commands(from group: JSONValue) -> JSONValue? {
    ...
    let filteredHooks = hooks.filter { hook in
      ...
      return !AgentHookCommandOwnership.isTty7ManagedCommand(command)
    }
