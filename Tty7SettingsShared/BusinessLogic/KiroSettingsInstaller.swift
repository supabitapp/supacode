...
nonisolated struct KiroSettingsInstaller {
  ...
  func installAllHooks() async throws {
    ...
    // already accepted a config from this tty7 build.
...
nonisolated enum KiroSettingsInstallerError: Error, Equatable, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .invalidJSON(let detail):
      "Kiro agent config must be valid JSON before tty7 can install hooks (\(detail))."
    case .invalidRootObject:
      "Kiro agent config must be a JSON object before tty7 can install hooks."
    case .kiroUnavailable:
      "Kiro must be installed and available in your login shell before tty7 can install hooks."
    case .unsupportedKiroVersion(let detected):
      """
      tty7 only knows Kiro \(KiroSettingsInstaller.supportedVersionPrefix)x defaults \
      (detected \(detected.isEmpty ? "unknown" : detected)). Update tty7 before installing hooks.
      """
    }
  }
}
