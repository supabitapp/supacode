...
struct AgentHookSettingsFileInstallerTests {
  ...
  private func makeTempURL() -> URL {
    URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("tty7-test-\(UUID().uuidString)")
  ...
  @Test func installPrunesLegacyCommands() throws {
        ...
        "Stop": .array([
          .object([
            "hooks": .array([
              .object([
                "type": "command",
                "command": "TTY7_CLI_PATH agent-hook --stop",
              ])
            ])
          ])
        ])
    ...
    for group in stopGroups {
      ...
      for hook in hooks {
        ...
        #expect(!cmd.contains("TTY7_CLI_PATH"))
      }
    }
  }
  ...
  @Test func uninstallRemovesEveryPreCollapseSentinelGroup() throws {
    ...
    let preCollapse: JSONValue = .object([
      "hooks": .object([
        "Notification": .array([
          .object(["hooks": .array([sentinelCommand("tty7 awaiting_input")]), "matcher": ""]),
          .object(["hooks": .array([sentinelCommand("tty7 notify")]), "matcher": ""]),
        ]),
        "Stop": .array([
          .object(["hooks": .array([sentinelCommand("tty7 idle")])]),
          .object(["hooks": .array([sentinelCommand("tty7 notify")])]),
        ]),
        "PreToolUse": .array([
          .object(["hooks": .array([userBashHook]), "matcher": "Bash"]),
          .object(["hooks": .array([sentinelCommand("tty7 busy")]), "matcher": ""]),
        ]),
      ])
    ])
    ...
    // PreToolUse keeps the user-authored Bash hook, drops the tty7 one.
  ...
  @Test func installPrunesStaleTty7Entries() throws {
        ...
        "Stop": .array([
          .object([
            "hooks": .array([
              .object([
                "type": "command",
                "command": .string("tty7 old-idle \(sentinel)"),
                "timeout": 5,
              ])
            ])
          ]),
          .object([
            "hooks": .array([
              .object([
                "type": "command",
                "command": .string("tty7 old-notify \(sentinel)"),
                "timeout": 10,
              ])
            ])
          ]),
