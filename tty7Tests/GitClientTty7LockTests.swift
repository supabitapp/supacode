...
struct GitClientTty7LockTests {
  @Test func reconcileBackfillsLockOnUnmanagedWorktree() async throws {
    ...
    await GitClient().reconcileTty7Locks(for: fixture.workURL)

    let reason = try String(contentsOf: lockFile, encoding: .utf8)
    let metadata = GitClient.parseTty7LockMetadata(from: reason)
    #expect(metadata?.owner == GitClient.tty7LockOwner)
    #expect(metadata?.createdAt != nil)
  }

  @Test func reconcilePreservesTty7LockWhenWorktreeMissing() async throws {
    let fixture = try GitWorktreeFixture()
    defer { fixture.cleanup() }
    ...
    GitClient.writeTty7Lock(at: adminDir)
  ...
  @Test func reconcileWorksWithProductionStyleRootURL() async throws {
    ...
    await GitClient().reconcileTty7Locks(for: productionStyleRoot)

    #expect(FileManager.default.fileExists(atPath: lockFile.path(percentEncoded: false)))
  }

  @Test func removeWorktreeReleasesTty7Lock() async throws {
    let fixture = try GitWorktreeFixture()
    defer { fixture.cleanup() }
  ...
  @Test func writeTty7LockProducesParseableMetadata() throws {
    let fixture = try GitWorktreeFixture()
    defer { fixture.cleanup() }
    ...
    let lockFile = adminDir.appending(path: "locked")
    let reason = try String(contentsOf: lockFile, encoding: .utf8)
    let metadata = try #require(GitClient.parseTty7LockMetadata(from: reason))
    #expect(metadata.owner == GitClient.tty7LockOwner)
    #expect(metadata.createdAt != nil)
  }

  @Test func lockPayloadRoundTripsThroughParser() throws {
    let payload = GitClient.currentTty7LockPayload()
    let metadata = try #require(GitClient.parseTty7LockMetadata(from: payload))
    #expect(metadata.owner == GitClient.tty7LockOwner)
    #expect(metadata.createdAt != nil)
  }
  ...
  @Test func parserRejectsForeignOwner() {
    ...
    #expect(GitClient.parseTty7LockMetadata(from: foreign) == nil)
  }

  @Test func parserRejectsNonJSONReason() {
    #expect(GitClient.parseTty7LockMetadata(from: "Managed by tty7") == nil)
    #expect(GitClient.parseTty7LockMetadata(from: "") == nil)
  }

  @Test func reconcileBackfillsLockOnBareRepository() async throws {
    let tempRoot = URL(filePath: "/tmp", directoryHint: .isDirectory)
    let id = UUID().uuidString
    let containerURL = tempRoot.appending(path: "tty7-bare-\(id)", directoryHint: .isDirectory)
    ...
    await GitClient().reconcileTty7Locks(for: bareURL)
