...
struct AgentHookCommandTests {
  ...
  @Test func compositeGuardsOnSurfaceOnly() {
    ...
    // is just the surface id (the no-op-outside-tty7 gate). The token and the
    // worktree / tab ids the socket envelope carried are gone.
    let command = AgentHookSettingsCommand.compositeCommand(
      events: [.busy], forwardStdinAsNotification: false, agent: .claude)
    #expect(command.contains("TTY7_SURFACE_ID"))
    #expect(!command.contains("TTY7_OSC_TOKEN"))
    #expect(!command.contains("token="))
    #expect(!command.contains("TTY7_WORKTREE_ID"))
    #expect(!command.contains("TTY7_TAB_ID"))
  }
  ...
  @Test func currentCommandIsRecognized() {
    let command = AgentHookSettingsCommand.compositeCommand(
      events: [.busy], forwardStdinAsNotification: false, agent: .claude)
    #expect(AgentHookCommandOwnership.isTty7ManagedCommand(command))
  }
  ...
  @Test func legacyCommandIsRecognized() {
    let legacy = "TTY7_CLI_PATH=/usr/bin/tty7 agent-hook --stop"
    #expect(AgentHookCommandOwnership.isTty7ManagedCommand(legacy))
    #expect(AgentHookCommandOwnership.isLegacyCommand(legacy))
  }

  @Test func legacyCommandRequiresBothMarkers() {
    #expect(!AgentHookCommandOwnership.isLegacyCommand("TTY7_CLI_PATH only"))
    #expect(!AgentHookCommandOwnership.isLegacyCommand("agent-hook only"))
  }

  @Test func unrelatedCommandIsNotRecognized() {
    #expect(!AgentHookCommandOwnership.isTty7ManagedCommand("echo hello"))
    #expect(!AgentHookCommandOwnership.isTty7ManagedCommand(nil))
  }
  ...
  @Test func userAuthoredCommandReferencingSocketEnvVarIsNotOwned() {
    ...
    // `TTY7_SOCKET_PATH` env var must NOT be classified as
    // tty7-managed, otherwise install would silently strip it.
    let userHook = #"echo "saw $TTY7_SOCKET_PATH" >> ~/my-debug.log"#
    #expect(!AgentHookCommandOwnership.isTty7ManagedCommand(userHook))
  ...
  @Test func userAuthoredHookFollowingDocumentedSocketPatternIsNotOwned() {
    ...
    let userHook =
      #"[ -n "$TTY7_SOCKET_PATH" ] && echo "x" | /usr/bin/nc -U -w1 "$TTY7_SOCKET_PATH" || true"#
    #expect(!AgentHookCommandOwnership.isTty7ManagedCommand(userHook))
    #expect(!AgentHookCommandOwnership.isLegacyCommand(userHook))
  }
  ...
  @Test func verbatimEnvCheckGuardWithoutSentinelIsLegacy() {
    ...
    // pre-sentinel tty7 hook and must be pruned on install/uninstall.
    let legacy =
      AgentHookSettingsCommand.envCheck
      + #" && echo "$TTY7_WORKTREE_ID $TTY7_TAB_ID $TTY7_SURFACE_ID 0""#
      + #" | /usr/bin/nc -U -w1 "$TTY7_SOCKET_PATH" 2>/dev/null || true"#
    #expect(AgentHookCommandOwnership.isLegacyCommand(legacy))
    #expect(AgentHookCommandOwnership.isTty7ManagedCommand(legacy))
  }
  ...
  @Test func legacyCLIShimSessionEventCommandIsRecognized() {
    ...
    // direct-nc era) shelled out to `tty7 integration event`.
    // Strip-on-update must still recognise it as tty7-managed,
    // otherwise the canonical hook is appended on top instead of
    // replacing it, producing duplicate SessionStart hooks.
    let legacy =
      #"[ -n "${TTY7_SOCKET_PATH:-}" ] && tty7 integration event session_start"#
  ...
  @Test func socketPathGatesThePresencePidSuffixOnly() {
    // `TTY7_SOCKET_PATH` survives in the command solely as the local-host
  ...
  @Test func compositeByteSnapshot_claudeBusy_inlineLiteral() {
    ...
    let expected =
      ...
      + #"__sp=""; [ -n "${TTY7_SOCKET_PATH:-}" ] && __sp=";pid=$PPID"; "#
      + #"printf '\033]3008;start=claude;event=busy%s\033\\' "$__sp" > "$__tty"; "#
      + #"} >/dev/null 2>&1 || true # tty7-managed-hook"#
    #expect(composite == expected)
  }
  ...
  @Test func compositeEmitsOSCPresenceGuardedBySurface() {
    ...
    // tty7). It fires local and remote alike, and carries no token.
    #expect(command.contains("]3008;start=claude;event=busy"))
    #expect(command.contains(#"[ -n "${TTY7_SURFACE_ID:-}" ]"#))
    #expect(!command.contains("token="))
    #expect(command.contains(#"> "$__tty""#))
    #expect(command.contains("ps -o tty="))
    #expect(!command.contains(#"[ -z "${TTY7_SOCKET_PATH:-}" ]"#))
  }
  ...
  @Test func presenceCarriesLocalPidButNotRemote() throws {
    ...
    // TTY7_SOCKET_PATH is set (local host), absent over SSH. A regression
    // that always or never emitted it would silently break the liveness sweep.
    let base: [String: String] = ["TTY7_SURFACE_ID": UUID().uuidString]
    let command = AgentHookSettingsCommand.compositeCommand(
      events: [.busy], forwardStdinAsNotification: false, agent: .claude)

    // Local (socket present): the presence OSC carries a positive pid.
    let local = try runHookCommandCapturingTTY(
      command, env: base.merging(["TTY7_SOCKET_PATH": "/tmp/sock-\(UUID().uuidString)"]) { $1 })
  ...
  @Test func emitsNothingOutsidetty7() throws {
    // No TTY7_SURFACE_ID = not a tty7 surface: the guard short-circuits
    // and the command writes nothing to the tty (the inert-outside-tty7
    // contract).
    let command = AgentHookSettingsCommand.compositeCommand(
  ...
  @Test func presenceOSCRoundTripsThroughParser() throws {
    ...
    let captured = try runHookCommandCapturingTTY(
      AgentHookSettingsCommand.compositeCommand(
        events: [.sessionStart], forwardStdinAsNotification: false, agent: .claude),
      env: [
        "TTY7_SURFACE_ID": surfaceID.uuidString,
        "TTY7_SOCKET_PATH": "/tmp/tty7-rt-\(UUID().uuidString)",
      ]
    )
  ...
  private static let guardAndTTY =
    ...
    + #"__tty=$(ps -o tty= -p "$PPID" 2>/dev/null | tr -d '[:space:]'); "#
    + #"case "$__tty" in *[0-9]*) __tty="/dev/${__tty#/dev/}";; *) __tty="/dev/tty";; esac; "#
  private static let suppressTail = #"} >/dev/null 2>&1 || true # tty7-managed-hook"#
  ...
  private func runHookCommandCapturingTTY(
    ...
    let workDir = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("tty7-hook-tty-\(UUID().uuidString)", isDirectory: true)
    ...
    // The host may already export tty7-surface vars (tests can run inside a
    // tty7 surface); clear them so every absent-variable assertion is genuine.
    environment.removeValue(forKey: "TTY7_SOCKET_PATH")
    environment.removeValue(forKey: "TTY7_SURFACE_ID")
