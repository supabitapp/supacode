...
struct WorktreeEnvironmentTests {
  @Test func scriptEnvironmentContainsExpectedKeys() {
    ...
    #expect(env["TTY7_WORKTREE_PATH"] == "/tmp/repo/wt-1")
    #expect(env["TTY7_ROOT_PATH"] == "/tmp/repo")
    #expect(env.count == 2)
  }
  ...
  @Test func blockingScriptLaunchWritesScriptAndMetadataFiles() throws {
    ...
    #expect(runnerScript.contains("TTY7_SHELL_PATH_FILE=\(quotedShellPath)") == true)
    #expect(runnerScript.contains("\"$TTY7_SHELL_PATH\" -l \(quotedScriptPath)") == true)
  ...
  @Test func blockingScriptLaunchPropagatesNonZeroExitCodeInZsh() throws {
    ...
    let tempHome = URL(
      fileURLWithPath: "/tmp/tty7-zsh-home-\(UUID().uuidString.lowercased())",
      isDirectory: true
    )
    ...
    // stdin so the `else exit "$TTY7_EXIT"` branch wins under xctest.
  ...
  @Test func blockingScriptCommandInputHandlesQuotedTempPathsInZsh() throws {
    ...
    let baseDirectoryURL = fileManager.temporaryDirectory.appending(
      path: "tty7 temporary path's with spaces \(UUID().uuidString.lowercased())",
    ...
    let tempHome = fileManager.temporaryDirectory.appending(
      path: "tty7-zsh-home-\(UUID().uuidString.lowercased())",
      directoryHint: .isDirectory
    )
  ...
  @Test func blockingScriptRunnerEmitsCommandFinishedUnderRealPTYBeforeExecTail() throws {
    ...
    let tempHome = FileManager.default.temporaryDirectory.appending(
      path: "tty7-pty-home-\(UUID().uuidString.lowercased())",
      directoryHint: .isDirectory
    )
    try FileManager.default.createDirectory(at: tempHome, withIntermediateDirectories: true)
    defer {
