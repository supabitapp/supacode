...
struct SidebarPersistenceMigratorTests {
  @Test(.dependencies) func noopWhenSidebarFileAlreadyExists() throws {
    ...
    try storage.save(seededBytes, Tty7Paths.sidebarURL)
    let existingBytes = try storage.load(Tty7Paths.sidebarURL)
    ...
    withDependencies {
      ...
      #expect((try? storage.load(Tty7Paths.sidebarURL)) == existingBytes)
  ...
  @Test(.dependencies) func migratesCollapsePinOrderArchiveFocus() throws {
    ...
    try withDependencies {
      ...
      let data = try storage.load(Tty7Paths.sidebarURL)
  ...
  @Test(.dependencies) func migratorRerunsWhenFileHasSchemaVersionZero() throws {
    ...
    try storage.save(priorBytes, Tty7Paths.sidebarURL)
  ...
  @Test(.dependencies) func migratorSkipsWhenFileAlreadyHasSchemaVersion1() throws {
    ...
    try withDependencies {
      ...
      #expect((try? storage.load(Tty7Paths.sidebarURL)) == seededBytes)
  ...
  @Test(.dependencies) func rescuesOrphanPinnedViaDefaultWorktreeBaseConvention() throws {
    // T6 — default `~/.tty7/repos/<name>/` convention. The
    ...
    // exact path from `Tty7Paths` so the expectation matches
    // whatever `worktreeBaseDirectory(...)` produces at runtime.
    let conventionBase = Tty7Paths.worktreeBaseDirectory(
      for: rootURL,
  ...
  @Test(.dependencies) func rescuesOrphanPinnedViaGlobalWorktreeBaseOverride() throws {
    ...
    let overrideBase = Tty7Paths.worktreeBaseDirectory(
      for: rootURL,
  ...
  @Test(.dependencies) func rescuesOrphanPinnedViaPerRepoWorktreeBaseOverride() throws {
    // T8 — per-repo `tty7.json` override. The migrator reads
    ...
    // Seed the per-repo `tty7.json` via the injected storage
    ...
    try localSettingsStorage.save(
      ...
      at: Tty7Paths.repositorySettingsURL(for: rootURL)
    )
  ...
  @Test(.dependencies) func rescuesOrphanArchivedViaDefaultWorktreeBaseConvention() throws {
    ...
    // `~/.tty7/repos/<name>/` with a timestamp should land in
