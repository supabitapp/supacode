...
struct GhosttySurfaceBridgeTests {
  ...
  func openUrlRequestPreservesHTTPSURL() {
    let request = ghosttyOpenURLRequest(
      urlString: "https://tty7.dev/changelog",
      kind: GHOSTTY_ACTION_OPEN_URL_KIND_UNKNOWN
    )

    #expect(request?.kind == .unknown)
    #expect(request?.url.absoluteString == "https://tty7.dev/changelog")
  ...
  func openUrlRequestTreatsTildePathAsFileURL() {
    let request = ghosttyOpenURLRequest(
      urlString: "~/code/github.com/supabitapp/tty7",
      kind: GHOSTTY_ACTION_OPEN_URL_KIND_UNKNOWN
    )
    ...
    #expect(
      request?.url.path
        ...
        .appending(path: "code/github.com/supabitapp/tty7").path
    )
  }
  ...
  func openUrlRequestExpandsNamedTildePathAsFileURL() {
    ...
    let input = "~\(username)/code/github.com/supabitapp/tty7"
    let request = ghosttyOpenURLRequest(
      urlString: input,
      kind: GHOSTTY_ACTION_OPEN_URL_KIND_UNKNOWN
    )
