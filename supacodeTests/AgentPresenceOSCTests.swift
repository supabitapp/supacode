...
struct AgentPresenceOSCTests {
  ...
  @Test func presenceEventThreadsLocalPid() {
    ...
    let result = WorktreeSurfaceState.presenceEvent(
  ...
  @Test func notificationExtractsBody() {
    let resolved = WorktreeSurfaceState.notification(
  ...
  @Test func notificationDropsUnknownSurface() {
    let result = WorktreeSurfaceState.notification(
  ...
  @Test func sanitizeStripsControlCharsAndCollapsesNewlines() {
    ...
    #expect(WorktreeSurfaceState.sanitizeNotificationText(dirty, max: 1000) == "a[31mred line")
  }

  @Test func sanitizeCapsToMaxScalars() {
    let long = String(repeating: "x", count: 500)
    #expect(WorktreeSurfaceState.sanitizeNotificationText(long, max: 100).count == 100)
  }
  ...
  @Test func notificationStripsEmbeddedOSCSequenceFromBody() {
    ...
    #expect(
      WorktreeSurfaceState.sanitizeNotificationText(dirty, max: 1000)
  ...
  @Test func largeNotifyPayloadNearMetadataCapRoundTripsEndToEnd() {
    ...
    // Body is sanitized + capped at 1000 scalars (see WorktreeSurfaceState.notification).
