...
// for every worktree tty7 manages. Detection keys on `owner ==
// "tty7"`; the other fields are forensic so a stranded lock file
// can be traced back to a specific build.
nonisolated struct Tty7WorktreeLockMetadata: Codable, Sendable, Equatable {
  let owner: String
  let version: String?
  let build: String?
  let createdAt: Int64?
...
struct GitClient {
  nonisolated static let tty7LockOwner = "tty7"
  ...
  // Backfill-only: never drop tty7-owned locks here. tty7-initiated
  // `removeWorktree` is the sole release path.
  nonisolated func reconcileTty7Locks(for repoRoot: URL) async {
    ...
    for entry in entries {
      ...
      Self.writeTty7Lock(at: entry.adminDirectory)
      gitLogger.info(
        "Backfilled tty7 lock for worktree \(entry.worktreeDirectory.lastPathComponent)"
      )
    }
    do {
      _ = try await runGit(
  ...
  nonisolated static func writeTty7Lock(at adminDirectory: URL) {
    let lockFile = adminDirectory.appending(path: "locked")
    try? currentTty7LockPayload().write(to: lockFile, atomically: true, encoding: .utf8)
  }

  nonisolated static func removeTty7Lock(at adminDirectory: URL) {
    let lockFile = adminDirectory.appending(path: "locked")
    let path = lockFile.path(percentEncoded: false)
    guard FileManager.default.fileExists(atPath: path) else { return }
    // Don't strip a user's `git worktree lock --reason "..."`; only
    // remove the file when it parses as a tty7-owned payload.
    guard let raw = try? String(contentsOf: lockFile, encoding: .utf8),
      parseTty7LockMetadata(from: raw) != nil
    else { return }
    try? FileManager.default.removeItem(at: lockFile)
  }

  // Stamp version/build for forensics on stranded lock files.
  nonisolated static func currentTty7LockPayload() -> String {
    let info = Bundle.main.infoDictionary
    let metadata = Tty7WorktreeLockMetadata(
      owner: tty7LockOwner,
    ...
    guard let data = try? encoder.encode(metadata),
      ...
      return Self.minimalTty7LockPayload
    }
    return payload
  }

  nonisolated private static let minimalTty7LockPayload = #"{"owner":"tty7"}"#

  nonisolated static func parseTty7LockMetadata(
    from reason: String
  ) -> Tty7WorktreeLockMetadata? {
    let trimmed = reason.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty, let data = trimmed.data(using: .utf8) else { return nil }
    guard let metadata = try? JSONDecoder().decode(Tty7WorktreeLockMetadata.self, from: data)
    else {
      return nil
    }
    return metadata.owner == tty7LockOwner ? metadata : nil
  }
            ...
            for try await streamEvent in shell.runLoginStream(
              ...
              case .finished(let output):
                ...
                if let adminDir = Self.adminDirectory(forWorktreeAt: worktreeURL) {
                  Self.writeTty7Lock(at: adminDir)
                }
  ...
  nonisolated func removeWorktree(_ worktree: Worktree, deleteBranch: Bool) async throws -> URL {
    ...
    await releaseTty7Lock(forWorktreeAt: worktreeURL, repoRoot: worktree.repositoryRootURL)
  ...
  nonisolated private func releaseTty7Lock(
    forWorktreeAt worktreeURL: URL,
    repoRoot: URL
  ) async {
    if let adminDir = Self.adminDirectory(forWorktreeAt: worktreeURL) {
      Self.removeTty7Lock(at: adminDir)
      return
    }
    ...
    Self.removeTty7Lock(at: match.adminDirectory)
  }
