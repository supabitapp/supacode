...
private enum GhosttyCLI {
  static let argv: [UnsafeMutablePointer<CChar>?] = {
    ...
    let executable = CommandLine.arguments.first ?? "tty7"
...
@MainActor
final class Tty7AppDelegate: NSObject, NSApplicationDelegate {
...
@MainActor
struct Tty7App: App {
  @NSApplicationDelegateAdaptor(Tty7AppDelegate.self) private var appDelegate
  ...
  @MainActor init() {
    // Carry a pre-rename state dir (`~/.amux` / `~/.supacode`) over to
    // `~/.tty7` before any @Shared persistence key (sidebar / layouts /
    // settings) reads it.
    Tty7Paths.migrateLegacyStateDirectoryIfNeeded()
  ...
  private static func makeStore(
    ...
    Store(initialState: AppFeature.State(settings: SettingsFeature.State(settings: initialSettings))) {
      ...
      // `@Shared(.sidebar)` bucket into the `Tty7SettingsShared`
    ...
    .commands {
      ...
      CommandGroup(replacing: .help) {
        Button("Submit GitHub Issue") {
          guard let url = URL(string: "https://github.com/supabitapp/tty7/issues/new") else { return }
