...
final class GhosttyRuntime {
  ...
  /// Applies tty7-specific config (padding values, transparent surface)
  ...
  ///
  /// `adjust-cell-height = -8%` removes the font's ~1.6px line gap from the
  /// cell so the row height is just ascent+descent. The default gap makes the
  /// monospace cell read tall/narrow, which visibly stretches block / ASCII
  /// art (e.g. the fastfetch logo) vertically.
  ///
  ...
  private static func loadBundledOverrides(into config: ghostty_config_t) {
    let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("tty7-defaults.conf")
    do {
  ...
  /// Loads the bundled tty7 light/dark theme plus its opacity and blur. No-op when sync is disabled.
  private static func loadBundledTheme(into config: ghostty_config_t, enabled: Bool) {
    guard enabled else { return }
    guard
      let lightPath = Bundle.main.path(forResource: "tty7 Light", ofType: nil),
      let darkPath = Bundle.main.path(forResource: "tty7 Dark", ofType: nil)
    else {
      assertionFailure("Bundled tty7 themes missing from app bundle.")
      logger.warning("Bundled tty7 themes missing from app bundle.")
      return
    }
    let contents = """
      ...
      """
    let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("tty7-theme.conf")
    do {
  ...
  // Uses the app's effective appearance — fine for tty7's single-window
...
extension NSPasteboard {
  ...
  static var pastedImagesDirectory: URL {
    FileManager.default.temporaryDirectory
      .appending(path: "tty7-pasted-images", directoryHint: .isDirectory)
  }
