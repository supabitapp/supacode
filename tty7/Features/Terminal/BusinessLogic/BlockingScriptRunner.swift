...
enum BlockingScriptRunner {
  ...
  static func makeLaunch(
    ...
    let directoryURL = baseDirectoryURL.appending(
      path: "tty7-blocking-script-\(UUID().uuidString.lowercased())",
  ...
  static func runnerScript(scriptURL: URL, shellPathURL: URL) -> String {
    ...
    return """
      ...
      set -u
      TTY7_EXIT=127
      trap 'printf "\\033]133;D;%d\\007" "$TTY7_EXIT"' EXIT
      trap 'exit 127' INT TERM HUP QUIT PIPE
      printf '\\033]133;C\\007'
      TTY7_SHELL_PATH_FILE=\(quotedShellPath)
      if [ ! -r "$TTY7_SHELL_PATH_FILE" ]; then
        printf '\\r\\nerror: missing shell-path file\\r\\n' >&2
        if [ -t 0 ]; then
          trap - EXIT
          printf '\\033]133;D;%d\\007' "$TTY7_EXIT"
          printf '\\r\\n\\033[2m── Script aborted (missing shell-path). Tab is read-only. ──\\033[0m\\r\\n'
          exec tail -f /dev/null
        fi
        exit 127
      fi
      IFS= read -r TTY7_SHELL_PATH < "$TTY7_SHELL_PATH_FILE" || exit 127
      "$TTY7_SHELL_PATH" -l \(quotedScriptPath)
      TTY7_EXIT=$?
      trap - EXIT
      printf '\\033]133;D;%d\\007' "$TTY7_EXIT"
      if [ -t 0 ]; then
