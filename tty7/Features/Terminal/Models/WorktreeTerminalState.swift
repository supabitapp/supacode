...
final class WorktreeTerminalState {
  ...
  /// Per-surface tty7 observables. `@ObservationIgnored` so dict churn
  ...
  private func surfaceEnvironment(tabId: TerminalTabID, surfaceID: UUID) -> [String: String] {
    ...
    env["TTY7_REPO_ID"] = percentEncode(repoPath, allowedCharacters: percentEncodingSet, label: "TTY7_REPO_ID")
    env["TTY7_WORKTREE_ID"] = percentEncode(
      worktree.id, allowedCharacters: percentEncodingSet, label: "TTY7_WORKTREE_ID")
    env["TTY7_TAB_ID"] = tabId.rawValue.uuidString
    env["TTY7_SURFACE_ID"] = surfaceID.uuidString
    if let socketPath {
      env["TTY7_SOCKET_PATH"] = socketPath
    }
    ...
    // Propagate the app's state-dir override so the bundled `tty7` CLI and
    // any tooling spawned in the shell resolve the same `~/.tty7` (or its
    // `TTY7_HOME` override) the app itself uses, keeping a dev build's
    // state isolated end-to-end.
    if let tty7Home = ProcessInfo.processInfo.environment["TTY7_HOME"], !tty7Home.isEmpty {
      env["TTY7_HOME"] = tty7Home
    }
    // Prepend the bundled CLI binary directory to PATH so that `tty7`
