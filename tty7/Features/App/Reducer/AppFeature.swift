...
struct AppFeature {
    ...
    let core = Reduce<State, Action> { state, action in
      ...
      case .deeplinkReceived(let url, let source, let responseFD):
        ...
        guard state.repositories.isInitialLoadComplete else {
          ...
          if let responseFD {
            return sendSocketResponse(
              clientFD: responseFD, ok: false, error: "tty7 is still loading. Try again.")
          }
  ...
  private func quitEffect(state: inout State, terminateSessions: Bool) -> Effect<Action> {
    ...
    let pendingFDEffect = drainPendingResponseFD(state: &state, error: "tty7 is quitting.")
  ...
  /// Builds a `tty7://worktree/<id>/surface/<tabID>/<surfaceID>` URL for a
  ...
  private func surfaceDeeplinkURL(worktreeID: Worktree.ID, surfaceID: UUID) -> URL? {
    ...
    guard let tabID = terminalClient.tabID(worktreeID, surfaceID) else {
      ...
      return urlOrWarn(
        "tty7://worktree/\(encodedWorktreeID)",
        worktreeID: worktreeID,
        surfaceID: surfaceID
      )
    }
    ...
    return urlOrWarn(
      "tty7://worktree/\(encodedWorktreeID)/tab/\(tabRaw)/surface/\(surfaceRaw)",
      worktreeID: worktreeID,
      surfaceID: surfaceID
    )
  }
