  ...
  var body: some View {
    Form {
      Section {
        ...
        Text("Symlinks `tty7` to `/usr/local/bin`. This is not required to run `tty7` in the app terminals.")
      }
      Section {
      ...
      Section {
        Toggle(isOn: $store.autoUpdateAgentIntegrationsEnabled) {
          ...
          Text(
            "Re-installs hooks for any agent reporting an outdated integration when tty7 comes to the foreground.")
        }
        .help("Silently re-applies the canonical hook layout to outdated agent integrations when tty7 activates.")
      }
    }
    .formStyle(.grouped)
    .padding(.top, -20)
