...
///
/// Writes to:
/// - `~/.tty7/sidebar.json` via the shared
...
enum SidebarPersistenceMigrator {
  ...
  static func migrateIfNeeded(
    ...
    let sidebarURL = Tty7Paths.sidebarURL
    ...
    // base, default `~/.tty7/repos/<name>/` convention) so
  ...
  ///   `Tty7Paths.worktreeBaseDirectory(for:globalDefaultPath:
  ///   repositoryOverridePath:)`, which routes through (in priority
  ///   order) per-repo `tty7.json` override → global
  ///   `defaultWorktreeBaseDirectoryPath` override → default
  ///   `~/.tty7/repos/<lastPathComponent>/` convention.
  ...
  /// corrupt `tty7.json` files are skipped silently; the
  ...
  private static func rootCandidates(
    ...
    for owningRoot in legacyRoots {
      ...
      //    `~/.tty7/repos/<name>/` convention when no override
      //    is configured.
      let worktreeBase = Tty7Paths.worktreeBaseDirectory(
        for: rootURL,
  ...
  /// Synchronously reads `<root>/tty7.json` via the injected
  ...
  private static func loadRepositoryOverridePath(
    ...
    let url = Tty7Paths.repositorySettingsURL(for: rootURL)
  ...
  /// `~/.tty7/repos/<name>/` still resolves to the settings
