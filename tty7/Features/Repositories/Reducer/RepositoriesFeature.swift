...
struct RepositoriesFeature {
  ...
  struct State: Equatable {
    ...
    /// worktree — persisted to `~/.tty7/sidebar.json`. Replaces
    ...
    Reduce { state, action in
      ...
      case .openRepositoriesFinished(let repositories, let failures, let invalidRoots, let roots):
        ...
        if !invalidRoots.isEmpty {
          let message = invalidRoots.map { "tty7 couldn't read \($0)." }.joined(separator: "\n")
      ...
      case .promptedWorktreeCreationDataLoaded(
        ...
        let defaultWorktreeBaseDirectory = Tty7Paths.worktreeBaseDirectory(
      ...
      case .createWorktreeInRepository(
        ...
        let worktreeBaseDirectory = Tty7Paths.worktreeBaseDirectory(
          for: repository.rootURL,
        ...
        return .run { send in
          ...
          do {
            ...
            let worktreeDirectoryURL = Tty7Paths.resolvedWorktreeDirectory(
      ...
      case .requestDeleteSidebarItems(let targets):
        ...
        if validKinds == [.folder] {
          ...
          state.alert = AlertState {
            ...
            TextState(
              "Remove \(messageSubject)? Choose \"Remove from tty7\" to stop "
                + stayOnDiskCopy
          ...
          if let folderIntent, let owningRepo {
            if owningRepo.isGitRepository {
              ...
              state.alert = messageAlert(
                ...
                message: "tty7 stopped the removal because \(owningRepo.name) became a git "
      ...
      case .alert(.presented(.confirmDeleteRepository(let repositoryID))):
        ...
        state.alert = nil
        // Section-level removal — tty7 never nukes a git repo's
        ...
        guard let worktree = state.worktree(for: worktreeID),
          ...
          return .send(
            .presentAlert(
              ...
              message: "tty7 could not find a pull request for this worktree."
            )
          )
        }
          ...
          guard let url = URL(string: pullRequest.url) else {
            return .send(
              .presentAlert(
                ...
                message: "tty7 could not open the pull request URL."
              )
            )
          }
          ...
          guard let failingCheckDetailsURL, !failingCheckDetailsURL.isEmpty else {
            return .send(
              .presentAlert(
                ...
                message: "tty7 could not find a failing check URL."
              )
            )
          }
          return .run { send in
        ...
        case .openFailingCheckDetails:
          guard let failingCheckDetailsURL, let url = URL(string: failingCheckDetailsURL) else {
            return .send(
              .presentAlert(
                title: "Failing check not found",
                message: "tty7 could not find a failing check with details."
              )
            )
          }
          return .run { @MainActor _ in
            ...
            guard !branchName.isEmpty else {
              await send(
                .presentAlert(
                  ...
                  message: "tty7 could not determine the pull request branch."
                )
              )
              return
            }
              ...
              guard let run = try await githubCLI.latestRun(worktreeRoot, branchName) else {
                ...
                await send(
                  .presentAlert(
                    ...
                    message: "tty7 could not find any workflow runs for this branch."
                  )
                )
                return
              }
              guard run.conclusion?.lowercased() == "failure" else {
                ...
                await send(
                  .presentAlert(
                    ...
                    message: "tty7 could not find a failing workflow run to copy logs from."
                  )
                )
                return
              }
            ...
            do {
              ...
              guard run.conclusion?.lowercased() == "failure" else {
                ...
                await send(
                  .presentAlert(
                    title: "No failing workflow run",
                    message: "tty7 could not find a failing workflow run to re-run."
                  )
                )
                return
              }
  ...
  private func loadRepositories(_ roots: [URL], animated: Bool = false) -> Effect<Action> {
    ...
    return .run { [animated, roots] send in
      ...
      await withTaskGroup(of: Void.self) { group in
        for root in roots {
          group.addTask { await gitClient.reconcileTty7Locks(root) }
        }
      }
      let (repositories, failures) = await loadRepositoriesData(roots)
      await send(
    ...
    let fetchResults = await withTaskGroup(of: WorktreesFetchResult.self) { group in
      for root in roots {
        ...
        group.addTask {
          ...
          // (user trashed it from Finder while tty7 was
