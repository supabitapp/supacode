...
private struct SidebarGitRepositorySection: View {
  ...
  var body: some View {
    ...
    Section(isExpanded: repositoryExpansionBinding) {
      ...
      // Toggle collapse from anywhere on the header row, not just the native
      // disclosure chevron. The chevron and the trailing `.sectionActions`
      // (ellipsis / plus) render in their own slots, so they keep working.
      Button {
        repositoryExpansionBinding.wrappedValue.toggle()
      } label: {
      ...
      .buttonStyle(.plain)
      .help(repositoryExpansionBinding.wrappedValue ? "Collapse" : "Expand")
    }
    .sectionActions {
  ...
  var body: some View {
    ...
    .sectionActions {
      ...
      Menu {
        ...
        .help("Remove this repository from tty7. Files on disk are untouched.")
      } label: {
        Image(systemName: "ellipsis")
          .accessibilityLabel("Options")
          .frame(maxHeight: .infinity)
