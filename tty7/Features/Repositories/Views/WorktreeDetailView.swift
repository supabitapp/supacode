...
private struct FailedRepositoryDetailView: View {
  ...
  var body: some View {
    ...
    ContentUnavailableView {
      ...
      VStack(spacing: 6) {
        Text("Restore the repository to keep working here, or remove it from tty7.")
  ...
  var body: some View {
    ContentUnavailableView {
      ...
      Button("Delete Worktree…", systemImage: "trash", role: .destructive, action: requestDelete)
        .help("Delete this worktree from tty7.")
    }
    .frame(maxWidth: .infinity, maxHeight: .infinity)
  }
}
      ...
      if !folderRows.isEmpty {
        selectionSection(
          ...
          actions: isMixedKindSelection
            ? []
            : [
              "Remove selected from tty7 (\(deleteShortcut))",
              "Right-click any selected folder to remove them all from tty7.",
            ]
        )
      }
