...
/// Resolves a worktree ID from an explicit flag or `$TTY7_WORKTREE_ID`.
nonisolated func resolveWorktreeID(_ explicit: String?) throws -> String {
  guard let id = nonEmpty(explicit) ?? EnvironmentDefaults.worktreeID else {
    throw ValidationError(
      "Missing worktree ID. Pass -w <id> or run inside a tty7 terminal ($TTY7_WORKTREE_ID)."
    )
  }
  return id
}

/// Resolves a tab ID from an explicit flag or `$TTY7_TAB_ID`.
nonisolated func resolveTabID(_ explicit: String?) throws -> String {
  guard let id = nonEmpty(explicit) ?? EnvironmentDefaults.tabID else {
    throw ValidationError(
      "Missing tab ID. Pass -t <id> or run inside a tty7 terminal ($TTY7_TAB_ID)."
    )
  }
  return id
}

/// Resolves a surface ID from an explicit flag or `$TTY7_SURFACE_ID`.
nonisolated func resolveSurfaceID(_ explicit: String?) throws -> String {
  guard let id = nonEmpty(explicit) ?? EnvironmentDefaults.surfaceID else {
    throw ValidationError(
      "Missing surface ID. Pass -s <id> or run inside a tty7 terminal ($TTY7_SURFACE_ID)."
    )
  }
  return id
}

/// Resolves a repo ID from an explicit flag or `$TTY7_REPO_ID`, percent-encoded.
nonisolated func resolveRepoID(_ explicit: String?) throws -> String {
  ...
  guard let id = EnvironmentDefaults.repoID else {
    throw ValidationError(
      "Missing repo ID. Pass -r <id> or run inside a tty7 terminal ($TTY7_REPO_ID)."
    )
  }
  return id
}
...
nonisolated func validatedScriptID(_ raw: String) throws -> String {
  guard let uuid = UUID(uuidString: raw) else {
    throw ValidationError(
      "Invalid --script value: expected a UUID. Run `tty7 worktree script list` to list script IDs."
    )
  }
