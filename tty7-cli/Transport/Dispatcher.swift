...
/// Dispatches a deeplink URL to the running tty7 app via its Unix domain socket.
...
nonisolated enum Dispatcher {
  ...
  static func resolveSocket() throws -> String {
    // Inside a tty7 terminal — use the env var if the socket is still alive.
    ...
    if existing.count > 1 {
      throw SocketClient.Error.responseError(
        "Multiple tty7 instances found. Run inside a tty7 terminal or specify TTY7_SOCKET_PATH.\n"
    ...
    guard let path = try waitForSocket(timeout: 10.0) else {
      throw SocketClient.Error.responseError("Timed out waiting for tty7 to start.")
    }
    return path
  }
  ...
  private static func launchApp() throws {
    ...
    guard process.terminationStatus == 0 else {
      ...
      let message = detail.isEmpty ? "Failed to launch tty7 (exit \(process.terminationStatus))." : detail
  ...
  private static func waitForSocket(timeout: TimeInterval) throws -> String? {
    ...
    FileHandle.standardError.write(Data("Waiting for tty7 to start...\n".utf8))
