...
- Auto-dismiss of the highlight onboarding card is state-driven from `.sidebarGroupingTogglesChanged` in the reducer: when both grouping toggles are off and the card isn't already dismissed past relevance, the handler bumps `@Shared(.appStorage("highlightRelevantOnboardingDismissedAt"))` to `now`. The menu bindings just write through to @Shared so any entry point (menu, deeplink, defaults edit, programmatic) fires the dismiss.
