...
- The Delete Script for a folder runs through the existing `.requestDeleteWorktree` → `.deleteWorktreeConfirmed` → `.deleteScriptCompleted` pipeline; the handlers branch inside so `gitClient.removeWorktree` is never called for a folder and the success path forwards to `.repositoryRemoved`. `removingRepositoryIDs` is the source of truth for "this is a folder delete" so the intent survives a `git init` happening between confirmation and completion.
