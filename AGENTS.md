...
- The toolbar `ScriptMenu` filters globals through `WorktreeToolbarState.visibleGlobalScripts` — globals minus repo IDs (`merged` already drops the collisions) and minus empty-command globals so half-configured entries don't surface in N repo toolbars.
