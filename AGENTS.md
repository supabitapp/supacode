...
- If you need to aggregate across many leaves in a parent row (group header indicators, batch summaries), extract a dedicated subview that takes only `parentStore: StoreOf<RepositoriesFeature>` + `leafIDs: [SidebarItemID]` and does the per-id scope+read inside its own body. That isolates the re-render to the aggregator, not the surrounding row chrome. See `SidebarPathGroupAggregatedIndicators` and `SidebarHighlightTopView` in `Features/Repositories/Views/` for the canonical pattern.

## Highlight Relevant Sidebar Items

- The View-menu toggle `@Shared(.sidebarHighlightRelevant)` (default `true`) gates a `SidebarHighlightTopView` at the top of the sidebar that hoists rows into a "Pinned" section and a classification-driven "Active" section. Both sections live inside `List` as siblings of the regular per-repo sections. Each section is independently collapsible via `Section(isExpanded:)`, with the expansion state persisted through `@Shared(.sidebarHighlightPinnedExpanded)` and `@Shared(.sidebarHighlightActiveExpanded)` (both default `true` so the feature is discoverable on first launch).
- `SidebarActiveClassification` (`SidebarHighlightSectionsView.swift`) is a 10-bucket priority enum keyed off four leaf-local flags (`hasUnseenNotifications`, `hasAgentAwaitingInput`, `!state.agents.isEmpty`, `!runningScripts.isEmpty`). The `hasAgent` flag matches visible agent-badge presence (any tracked instance, including `.idle`) so a row with an agent badge surfaces in Active even when the agent isn't actively working. Rows that don't classify into one of the buckets are dropped from Active and kept at the bottom of Pinned alphabetically. `SidebarHighlightOrdering` is the pure helper that owns the priority + alphabetical sort; it has direct unit coverage.
- `SidebarHighlightTopView` is the canonical aggregator. It reads its own candidate IDs (`store.state.sidebarItems.ids` filtered by archive) inside its own body so the per-row observation never leaks back to `SidebarListView`. Each candidate is then per-leaf-scoped via `store.scope(state: \.sidebarItems[id: id])` to classify. Per-row state ticks (notification, running script, agent activity) only re-render the aggregator subview.
- `SidebarItemContainer` suppresses any row that's hoisted (`isPinned` or classifies into Active) when the toggle is on, so the same id never renders twice. The suppression is gated by `@Environment(\.sidebarHighlightRelevant)` set on the `List` once by `SidebarListView`.
- Highlight rows get a colored `repo · trail` subtitle. The subtitle composes inside an `HStack` with `.layoutPriority(1)` on the trail so the disambiguating worktree name doesn't get truncated first under a narrow sidebar.
- Folders are pinnable through the same `pinWorktree` / `unpinWorktree` actions as git worktrees. The pin / unpin flow uses `SidebarState.removeAnywhere` + `insert` to enforce the "exactly one bucket" invariant against any pre-state (hand-edit, migrator race) where a row lives in two buckets simultaneously.
