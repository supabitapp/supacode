...
extension RepositoriesFeature {
  ...
  /// instead of a generic "Action not available."
...
extension RepositoriesFeature {
  ...
  func consolidatedTrashFailureAlert(
    ...
    if count == 1, let (id, message) = failureMessagesByRepositoryID.first {
      let folderName = state.repositories[id: id]?.name ?? "the folder"
      return messageAlert(
        title: "Delete from disk failed",
        message: "Couldn't move \(folderName) to the Trash: \(message)"
      )
    }
    let lines = failureMessagesByRepositoryID
      .map { id, message -> String in
  ...
  func folderRemovalEffect(
    ...
    // `failureMessage` along so the aggregator can coalesce
    // parallel failures into a single alert instead of each
    // overwriting `state.alert`.
