...
struct CommandPaletteFeature {
  ...
  ///
  /// The current project (`projectMRU` head) is rendered so you can see
  /// where you are, but it carries `isCurrentProject` so the overlay skips
  /// it for the default selection — Cmd+P then Enter lands on the previous
  /// project (Cmd+Tab style) instead of being a no-op. Intra-project
  ...
  static func projectSwitcherItems(
    ...
    let currentProjectID = mruOrder.first
    ...
    return ordered.enumerated().map { index, repo in
      ...
      return CommandPaletteItem(
        ...
        priorityTier: index,
        isCurrentProject: repo.id == currentProjectID
      )
    }
  }
